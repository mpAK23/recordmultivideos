...
function initHost() {
    ...
    const roomId = 'ovd-' + Math.floor(1000 + Math.random() * 9000).toString(); // Easy 4 digit after prefix
    ...
    appState.peer.on('call', call => {
        // Answer incoming camera stream
        call.answer(); // Don't send our own stream
...
async function initCamera() {
    ...
    // We add the prefix internally if they forgot, or they typed it fully
    const hostId = rawId.startsWith('ovd-') ? rawId : 'ovd-' + rawId;
    ...
    try {
        ...
        appState.peer.on('open', () => {
            ...
            call.on('stream', () => {
                // Technically Host doesn't send a stream, so this won't trigger. 
                // But connection is open.
            });
...
async function toggleRecord(id) {
    ...
    // Start MediaRecorder
    // For wider compatibility, try webm or standard mp4 if native
...
function saveTrackData(id, blob) {
    ...
    vid.srcObject = null;
    vid.src = track.url;
    vid.muted = false; // We want to hear the recorded audio when played back
    
    // Setup Audio Routing for Export Mix
    ...
    if (!track.audioNode && appState.audioCtx) {
        ...
        track.audioNode.connect(appState.audioCtx.destination); // Route back to speakers
    }
...
async function exportMix() {
    ...
    // Create Media Destination Node
    const dest = appState.audioCtx.createMediaStreamDestination();
    
    // Connect selected tracks to Destination
    tracksToExport.forEach(t => {
        if(t.track.audioNode) {
            // Re-route slightly: connect to destination AND master output
    ...
    recorder.onstop = () => {
        // Reconnect audio nodes to normal speaker out
        tracksToExport.forEach(t => {
            if(t.track.audioNode) {
                t.track.audioNode.disconnect();
                t.track.audioNode.connect(appState.audioCtx.destination);
